...
import { useGetEventBookingsQuery } from '@/redux/api/BookingApi';
import { formatDateRange, formatIndianNumber } from '@/utils/Helpers';
import EventBookingTableShimmer from './EventBookingTableShimmer';

const EventBookingTable = () => {
    const [status, setStatus] = useState("")
    const [page, setPage] = useState(1)
    const eventbookingQuery = useGetEventBookingsQuery({ page, status })
    const loading = eventbookingQuery?.isLoading
    const isError = eventbookingQuery?.isError
    const events = eventbookingQuery?.data?.data?.eventbookings
    const pagination = eventbookingQuery?.data?.data?.pagination
